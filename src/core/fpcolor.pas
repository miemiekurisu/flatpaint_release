...
var
  EffectiveAlpha: Integer;
  InverseAlpha: Integer;
begin
  EffectiveAlpha := (Src.A * Opacity) div 255;
  if EffectiveAlpha <= 0 then
    Exit(Dst);
  if EffectiveAlpha >= 255 then
    Exit(RGBA(Src.R, Src.G, Src.B, 255));

  InverseAlpha := 255 - EffectiveAlpha;
  Result.R := ClampByte((Src.R * EffectiveAlpha + Dst.R * InverseAlpha) div 255);
  Result.G := ClampByte((Src.G * EffectiveAlpha + Dst.G * InverseAlpha) div 255);
  Result.B := ClampByte((Src.B * EffectiveAlpha + Dst.B * InverseAlpha) div 255);
  Result.A := ClampByte(EffectiveAlpha + ((Dst.A * InverseAlpha) div 255));
end;
