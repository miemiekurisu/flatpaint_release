...
- Current code status after latest implementation pass:
  ...
  - **A4**: partially mitigated; Phase-4.5 metadata foundation is complete (layer offset metadata in core model, native persistence, and XCF metadata capture), with compatibility render mode intentionally retained.
