...
26. Unit tests for core functions must include edge-case and extreme-value scenarios: inverted/out-of-bounds coordinates, zero-size inputs, parameters exceeding valid range (clamping), degenerate geometry, and all combine/blend modes.
27. No hardcoded magic numbers in application or core code. Every numeric literal that encodes a domain-specific meaning (algorithm weights, threshold caps, angular constants, sentinel values, etc.) must be declared as a named constant with a descriptive identifier. Trivially clear values (0, 1, 255 as byte bounds, simple loop indices) are exempt; borderline cases should err on the side of naming. See `docs/MAGIC_NUMBERS_AND_DECOUPLING.md` for the project-wide inventory and naming conventions.
