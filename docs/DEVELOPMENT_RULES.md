...
20. A feature is not complete until the visible UI proves it: any tool, filter, or edit command that changes document state must also invalidate the relevant canvas/layer/tab preview surfaces and show an immediate on-screen result, not just mutate data silently.
21. Any color-sampling path that feeds a future paint action must preserve a visible paint alpha unless the UI explicitly exposes alpha sampling as a separate user choice; sampling a transparent pixel must not silently turn the next stroke into invisible paint.
22. Treat the default `Background` layer as a special non-transparent base layer, not as an ordinary alpha layer: any destructive tool or edit command that would normally punch transparency (`Eraser`, `Cut`, `Erase Selection`, moving selected pixels, equivalent clear paths) must restore an opaque background-color result on that layer instead of creating transparent holes.
